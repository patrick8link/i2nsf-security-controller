...
static void usage(char *cmd) {
    ...
    fprintf(stderr, "      --cisco-ios\n");
#endif
    fprintf(stderr, "      --no-backquotes   (only with --cisco or "
            "--cisco-ios)\n");
    fprintf(stderr, "      --with-defaults\n");
    fprintf(stderr, "      --show-defaults\n");
    fprintf(stderr, "      --cdb-config-oper\n");
    ...
    fprintf(stderr, "      --keep-pipe-flags\n");
...
int main(int argc, char **argv)
{
    ...
    while(1) {
        ...
        case NO_BACKQUOTE:
            ...
            load_flags |= MAAPI_CONFIG_NO_BACKQUOTE;
            save_flags |= MAAPI_CONFIG_NO_BACKQUOTE;
            break;
        ...
        case CISCO:
            ...
            break;
        case CISCO_IOS:
            style_flags |= MAAPI_CONFIG_C_IOS;
            break;
        case WITH_DEFAULTS:
            save_flags |= MAAPI_CONFIG_WITH_DEFAULTS;
            break;
        case CDB_CONFIG_OPER:
            save_flags |= MAAPI_CONFIG_CDB_ONLY+MAAPI_CONFIG_WITH_OPER;
            break;
        case SHOW_DEFAULTS:
    ...
    case SAVE:
        ...
        if (save_flags & MAAPI_CONFIG_NO_BACKQUOTE)
            if ((save_flags & MAAPI_CONFIG_C) == 0) {
                fprintf(stderr,
                        "Error: --no-backquotes only allowed with --cisco\n");
                usage(argv[0]);
                exit(2);
            }
    ...
    case LOADCMDS:
        ...
        if (load_flags & MAAPI_CONFIG_NO_BACKQUOTE)
            if ((load_flags & (MAAPI_CONFIG_C | MAAPI_CONFIG_C_IOS)) == 0) {
                fprintf(stderr,
                        "Error: --no-backquotes only allowed with --cisco\n");
                usage(argv[0]);
                exit(2);
