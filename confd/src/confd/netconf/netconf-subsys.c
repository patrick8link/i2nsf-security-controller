...
void xwrite(int fd, const char *buf, size_t nbytes_left)
{
    const char *pos = buf;
    do {
        ssize_t nbytes = write(fd, pos, nbytes_left);
        if(nbytes == -1) {
            if(errno != EAGAIN && errno != EINTR) {
                /* non retryable error */
                fatal(4, "Failed to write on fd %d: %d", fd, errno);
            }
        } else {
            nbytes_left -= nbytes;
            pos += nbytes;
...
static void creds(int sock)
{
    ...
    if ((n = getgroups(0, NULL)) < 0)
        exit(1);
    if (!(list = calloc(n, sizeof(gid_t))))
        exit(1);

    n = getgroups(n, list);
    xtragids = get_secondary_gids(gid, list, n);
    size_t buf_len = snprintf(NULL, 0, "[%s;%s/%s;ssh;%d;%d;%s;%s;",
                              user, ip, port,
                              getuid(), gid,
                              xtragids, homedir) + 1;
    /* if buf_len is less than BUFSIZ*2, then we allocate some more memory
       and hope we will not need to realloc later */
    if(buf_len < BUFSIZ * 2) {
        buf_len = BUFSIZ * 2;
    }
    char *buf = confd_xmalloc(buf_len);

    size_t num_written = snprintf(buf, buf_len, "[%s;%s/%s;ssh;%d;%d;%s;%s;",
                                  user, ip, port,
                                  getuid(), gid,
                                  xtragids, homedir);
    ...
    if (n > 0) {
        ...
        size_t group_name_len =
            confd_get_group_name_len(list, n, &grpbuf, &grpsize);
        groups = confd_build_group_names(list, n, group_name_len,
                                         &grpbuf, &grpsize);
        free(grpbuf);
        if(buf_len - num_written < group_name_len) {
            buf_len += group_name_len;
            buf = confd_xrealloc(buf, buf_len);
        }
        memcpy(buf + num_written, groups, group_name_len);
        num_written += group_name_len;
        free(groups);
    }
    ...
    char ending[] = ";]\n";
    size_t ending_length = sizeof(ending) - 1;
    if(buf_len - num_written < ending_length) {
        buf_len += ending_length;
        buf = confd_xrealloc(buf, buf_len);
    }

    memcpy(buf + num_written, ending, ending_length);
    num_written += ending_length;

    xwrite(sock, buf, num_written);
