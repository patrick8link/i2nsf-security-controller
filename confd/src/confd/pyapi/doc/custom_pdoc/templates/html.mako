<%
  ...
  def link(dobj: pdoc.Doc, name=None):
    ...
    if url.endswith('.ext'):
        if 'confd' in url or 'ncs' in url:
            return name
        if name in ['bool', 'int', 'float', 'complex', 'str']:
            url = 'https://docs.python.org/3/library/functions.html#' + \
                ('func-' if name == 'str' else '') + name
        else:
            url = 'https://docs.python.org/3/library/typing.html#typing.' + name
...
% else:
  ...
  % for name, desc in modules:
      <div class="flex">
      <dt><a href="${link_prefix}${name}/index.html">${name}</a></dt>
