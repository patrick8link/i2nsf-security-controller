...
static void *get_shm_addr(size_t offset)
{
    ...
    /* calculate heap pagesize-aligned address offset */
    addr = malloc(1);
    heapaddr = (uintptr_t)(addr+offset) - ((uintptr_t)(addr+offset) % pagesize);
    free(addr);
    return (void *)heapaddr;
}
    ...
    if (strcmp(unm.sysname, "Linux") == 0 ||
        ...
        shm_flags = 0;
              ...
              (strcmp(unm.machine, "x86_64") == 0 ||
               strcmp(unm.machine, "arm64") == 0)) {
        /* Darwin seems okay with heap top + 256 MB and without flags */
        shm_addr = get_shm_addr(256 * MB);
        shm_size = 0;
        shm_flags = 0;
    } else {
