...
static char prep_file[FNAME_SIZE];
static char ckp_file[FNAME_SIZE];
static char startup_file[FNAME_SIZE];
static FILE *estream;
...
static void err(void) { fprintf(stderr, "Return err\n"); }
...
static int restore(char *file)
{
    ...
    /* Now it's time to collect the users */
     while (fgets(&buf[0], BUFSIZ, fp) != NULL) {
         ...
         name = strtok_r(buf, sep, &saveptr);
         uid = strtok_r(NULL, sep, &saveptr);
         gid = strtok_r(NULL, sep, &saveptr);
         password = strtok_r(NULL, sep, &saveptr);
         ssh_keydir = strtok_r(NULL, sep, &saveptr);
         homedir = strtok_r(NULL, sep, &saveptr);
     ...
     while (fgets(&buf[0], BUFSIZ, fp) != NULL) {
          ...
          indexstr = strtok_r(buf, sep, &saveptr);
          context = strtok_r(NULL, sep, &saveptr);
          path = strtok_r(NULL, sep, &saveptr);
          group = strtok_r(NULL, sep, &saveptr);
          ops = strtok_r(NULL, sep, &saveptr);
          actionstr = strtok_r(NULL, sep, &saveptr);
      ...
      while (fgets(&buf[0], BUFSIZ, fp) != NULL) {
          ...
          indexstr = strtok_r(buf, sep, &saveptr);
          namespace = strtok_r(NULL, sep, &saveptr);
          context = strtok_r(NULL, sep, &saveptr);
          path = strtok_r(NULL, sep, &saveptr);
          group = strtok_r(NULL, sep, &saveptr);
          ops = strtok_r(NULL, sep, &saveptr);
...
static int tt_unlock(struct confd_trans_ctx *tctx)
{
    if (conf_file_lock_fd > 0) {
        IGNORE_RESULT( lockf(conf_file_lock_fd, F_ULOCK, 0) );
        close(conf_file_lock_fd);
    }
    return CONFD_OK;
}
...
int main(int argc, char *argv[]) {
    ...
    snprintf(prep_file, sizeof(prep_file), "%s.prep", conf_file);
    snprintf(ckp_file, sizeof(ckp_file), "%s.ckp", conf_file);
    snprintf(startup_file, sizeof(startup_file), "%s.startup", conf_file);
    if (!use_startup)
        unlink(startup_file);
    ...
    strncpy(data.callpoint, "aaa", sizeof(data.callpoint));
    ...
    while (1) {
        ...
        if (set[0].revents & POLLIN) {
            if (interactive) {
                char buf[BUFSIZ];
                IGNORE_RESULT( fgets(buf, BUFSIZ, stdin) );  /* clear fd */
                fpsave(stderr);
            }
            else {
