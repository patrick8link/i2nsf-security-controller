...
-spec load(Address, Port) -> Result when
      Address :: econfd:ip(),
...
-spec ikeypath2cs(IKeypath) -> Result when
...
%% @doc Get a flat list of children for a {@link confd_cs_node()},
%% with any choice/case structure(s) removed.
-spec choice_children(Node) -> Children when
...
%% (device data in NSO), TypeId must be {@link confd_cs_node()}.
-spec str2val(TypeId, Lexical) -> Result when
      TypeId :: confd_cs_node() | econfd:type(),
      Lexical :: binary(),
      Result :: {'ok', Value :: econfd:value()} |
...
-spec val2str(TypeId, Value) -> Result when
      TypeId :: confd_cs_node() | econfd:type(),
      Value :: econfd:value(),
...
-spec get_type(TypeName) -> Result when
      TypeName :: atom(),
...
-spec get_type(Ns, TypeName) -> econfd:type() when
      Ns :: econfd:namespace(),
      TypeName :: atom().
...
-spec register_type_cbs(TypeCbs) -> 'ok' when
      TypeCbs :: confd_type_cbs().
...
get_builtin_type('AES256CFB128IVEncryptedString') ->
    {?CONFD_URI, 'AES256CFB128IVEncryptedString'};
...
get_builtin_type('aes-cfb-128-encrypted-string') ->
    get_builtin_type('AESCFB128IVEncryptedString');
get_builtin_type('aes-256-cfb-128-encrypted-string') ->
    get_builtin_type('AES256CFB128IVEncryptedString');
