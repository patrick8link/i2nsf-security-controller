        ...
        confd_hkeypath_t *keypath, confd_value_t *newval)
{
    TRACE_ENTER("");
    char buf[BUFSIZ];
    const char *new_level_str, *compiled_level_str;
    int ret = CONFD_OK;
    ...
    if (new_level > get_trace_log_compiled_level()) {
        ...
        new_level_str =
            get_trace_level_name(new_level);
        compiled_level_str =
            get_trace_level_name(get_trace_log_compiled_level());
        confd_trans_seterr(tctx,
                           "Level %s cannot be set as it is more detailed than"
                           " compiled level %s!",
                           new_level_str, compiled_level_str);
        ret = CONFD_ERR;
    }
