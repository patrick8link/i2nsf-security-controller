...
int config_cdb_exists(confd_hkeypath_t *kp)
{
    int exists;
    int rsock = get_readsock(addr);
    if (cdb_start_session2(rsock, CDB_RUNNING, 0) != CONFD_OK) {
        error("Failed to start CDB RUNNING no lock session!");
        return -1;
    }
    exists = cdb_exists(rsock, "%h", kp);
    cdb_close(rsock);
    return exists;
}
