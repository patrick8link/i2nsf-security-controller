...
int switch_rt_leaves(
    ...
    // For get_attrs calls type may not be C_XMLTAG
    int pos = 0;
    if (kp->v[0][0].type != C_XMLTAG) {
        pos = 1;
    }
    int leaf_tag = CONFD_GET_XML(&kp->v[pos][0]);
...
//        next-hop/outgoing-interface
        ...
        case rt_routing:
            // Only used for get_attrs()
            CONFD_SET_XMLBEGIN(&v, rt_routing, rt__ns);
            break;
        ...
        default:
            warn("switch_rt_leaves: unimplemented yet %d", leaf_tag);
            ret_code = CONFD_OK;
            goto exit;
            break;
    }
...
int switch_ipv4_leaves(
    ...
    switch (leaf_tag) {
        ...
        default:
            warn("switch_ipv4_leaves: unimplemented yet %d", leaf_tag);
            ret_code = CONFD_OK;
            goto exit;
    }
...
int switch_ipv6_leaves(
    ...
    switch (leaf_tag) {
        ...
        default:
            warn("switch_ipv6_leaves: unimplemented yet %d", leaf_tag);
            ret_code = CONFD_OK;
            goto exit;
    }
...
// main callback registered for data provider .get_attrs
int routing_get_attrs(
  ...
  if (config_cdb_exists(kp) == 1) {
    idref.id = or_intended;
  }
  ...
  int leaf_ns = CONFD_GET_XML_NS(&kp->v[pos][0]);
  ...
  if (C_NOEXISTS != rv.type) {
      LOG("returning attribute for value %s", VALSTRNN(&rv));
...
int routing_get_next(
    ...
    switch (branch_tag) {
        case rt_interfaces:
            ret_code = if_status_get_next_name(tctx, kp, next);
            break;
        case rt_control_plane_protocols:
