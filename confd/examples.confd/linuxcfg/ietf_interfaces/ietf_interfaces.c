...
static int if_get_attrs(struct confd_trans_ctx *tctx,
                        confd_hkeypath_t *kp,
...
static struct confd_data_cbs transpoints[] = {
    { .callpoint = "if_state_dp", .get_attrs = if_get_attrs,
      .get_elem = if_get_elem, .get_next = if_get_next },
    { .callpoint = "" }
};
...
/* The read socket used by the get_attr() callback */
static int cdb_read_socket = 0;
                        ...
                        u_int32_t *attrs, int num_attrs)
{
  ...
  if (tag == if_interfaces) {
      if (kplen > 0) {
          tag = GET_NEXT_TAG;
          /* /interfaces/interface{X}/ operational-state */
          if (tag == if_interface) {
              idx = idx - 1;
              ifname = (char *) CONFD_GET_BUFPTR(&kp->v[idx][0]);
              p = iflist_find(ifname);
...
/* Data provider for /interfaces/ operationa-state */
static int if_get_elem(struct confd_trans_ctx *tctx,
                       confd_hkeypath_t *kp) {
    int tag, idx;
    if_entry_t *ife;
