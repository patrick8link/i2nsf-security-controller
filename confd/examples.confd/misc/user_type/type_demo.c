...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < ncbs; i++) {
        if (strcmp(cbs[i].typepoint, types__typepointid_port_type) == 0)
            OK(confd_register_ns_type(types__ns,"port-type",cbs[i].type));
        else if (strcmp(cbs[i].typepoint, types__typepointid_ipv4_mask) == 0)
            OK(confd_register_ns_type(types__ns,
                                      "ipv4-address-mask",cbs[i].type));
        else if (strcmp(cbs[i].typepoint, types__typepointid_ipv6_mask) == 0)
            OK(confd_register_ns_type(types__ns,
                                      "ipv6-address-mask",cbs[i].type));
    }
    ...
    for (i = 0; i < nports; i++) {
        ...
        type = confd_find_ns_type(types__ns, "ip-address-mask");
