...
int content_type_bit(confd_value_t *type_enum) {
    switch (CONFD_GET_ENUM_VALUE(type_enum)) {
    case folders_media:
        return storage__bm_content_type_media;
    case folders_document:
        return storage__bm_content_type_document;
    case folders_archive:
        return storage__bm_content_type_archive;
    }
    WARN("unknown content enum (%d)", CONFD_GET_ENUM_VALUE(type_enum));
    return 0;
}

int get_content_bits(
    struct confd_trans_ctx *tctx,
    confd_hkeypath_t *kp,
    confd_value_t *content_bits)
{

    confd_value_t v_storage_id, v_user_id;
    int ret;

    ret = get_content_bits_fields(tctx, kp, &v_user_id, &v_storage_id,
                                  content_bits);
    confd_free_value(&v_storage_id);
    return ret;
}

int get_content_bits_fields(
    struct confd_trans_ctx *tctx,
    confd_hkeypath_t *kp,
    confd_value_t *user_id,
    ...
    confd_value_t *content_bits)
{
    ...
    confd_value_t *folder_id = &kp->v[kp->len-4][0];
    int ret;

    get_user_id_by_username(tctx, username, user_id);
    get_ll_storage_id(tctx, username, folder_id, storage_id);
    ret = maapi_get_elem(glob.maapi_socket, tctx->thandle, content_bits,
                         OWNER_PATH "{%x %x}/content-type",
                         user_id, storage_id);
    if (ret != CONFD_OK && CONFD_ERR_NOEXISTS == confd_errno) {
        CONFD_SET_BIT32(content_bits, 0);
        ret = CONFD_OK;
    } else {
        OK(ret);
    }
