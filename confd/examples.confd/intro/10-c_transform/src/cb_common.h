...
// get the bit corresponing to the content-type enum value
int content_type_bit(confd_value_t *type_enum);

// get the LL bit value for the path
int get_content_bits(struct confd_trans_ctx *tctx,
                     confd_hkeypath_t *kp,
                     confd_value_t *content_bits);

// get the LL bit value for the path; also populate the two fields
