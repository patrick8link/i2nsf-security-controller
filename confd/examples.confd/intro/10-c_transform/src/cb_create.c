...
// creation of /../../content-type{}; flip the corresponding storage bit
static int create_content_type(struct confd_trans_ctx *tctx,
                               confd_hkeypath_t *kp) {
    TRACE_ENTER("");

    confd_value_t v_content_bits, v_user_id, v_storage_id;
    int ret;
    u_int32_t bits;

    get_content_bits_fields(tctx, kp, &v_user_id, &v_storage_id,
                            &v_content_bits);
    bits = CONFD_GET_BIT32(&v_content_bits) | content_type_bit(&kp->v[0][0]);
    CONFD_SET_BIT32(&v_content_bits, bits);
    OK(maapi_set_elem(glob.maapi_socket, tctx->thandle, &v_content_bits,
...
int cb_create(struct confd_trans_ctx *tctx, confd_hkeypath_t *kp)
{
    ...
    switch (list_tag) {
        ...
        case folders_content_type:
            ret = create_content_type(tctx, kp);
            break;
