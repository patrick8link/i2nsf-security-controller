...
"    -v 'verbosity' (verbosity for progress messages"
" - enum(normal|verbose|very-verbose|debug))\n"
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv,
                       "adtciuhsfSFNJWDHCUPyYren:AT:B:E:x:z:v:p:")) != -1) {
        switch(c) {
        ...
        case 'J': events |= CONFD_NOTIF_JSONRPC; break;
        case 'W': events |= CONFD_NOTIF_WEBUI; break;
        ...
        case 'v':
            if (strcmp(optarg, "normal") == 0) {
                dt.verbosity = CONFD_VERBOSITY_NORMAL;
            } else if (strcmp(optarg, "verbose") == 0) {
                dt.verbosity = CONFD_VERBOSITY_VERBOSE;
            } else if (strcmp(optarg, "very-verbose") == 0) {
                dt.verbosity = CONFD_VERBOSITY_VERY_VERBOSE;
            } else if (strcmp(optarg, "debug") == 0) {
                dt.verbosity = CONFD_VERBOSITY_DEBUG;
            } else {
                help(argv[0]);
            }
            break;
    ...
    while (1) {
        ...
        if (set[0].revents & POLLIN) {
            ...
            case CONFD_NOTIF_HA_INFO: {
                ...
                switch(hnot->type) {
                case CONFD_HA_INFO_NOPRIMARY: {
                    int error = hnot->data.noprimary;
                    /* why don't we have a primary */
                    switch (error) {
                    case CONFD_ERR_HA_NOTICK:
                        printf("NOPRIMARY: notick\n");
                        break;
                    case CONFD_ERR_HA_CLOSED:
                        printf("NOPRIMARY: closed\n");
                        break;
                    }
                    break;
                }
                case CONFD_HA_INFO_SECONDARY_DIED: {
                    char tbuf[255];
                    struct confd_ha_node *dst = &hnot->data.secondary_died;
                    confd_pp_value(tbuf, 255, &dst->nodeid);
                    printf("Secondary %s died\n", tbuf);
                    break;
                }
                case CONFD_HA_INFO_SECONDARY_ARRIVED: {
                    char tbuf[255];
                    struct confd_ha_node *dst = &hnot->data.secondary_arrived;
                    confd_pp_value(tbuf, 255, &dst->nodeid);
                    printf("Secondary %s arrived\n", tbuf);
                    break;
                }
                case CONFD_HA_INFO_SECONDARY_INITIALIZED:
                    printf("Secondary is initialized (cdbcopy=%d)\n",
                           hnot->data.cdb_initialized_by_copy);
                    break;
                case CONFD_HA_INFO_IS_PRIMARY:
                    printf("We are now primary \n");
                    break;
                case CONFD_HA_INFO_IS_NONE:
                    printf("We are now none \n");
                    break;
                case CONFD_HA_INFO_BESECONDARY_RESULT: {
                    if (hnot->data.besecondary_result == 0) {
                        printf("Async besecondary() succeeded\n");
                    } else {
                        printf("Async besecondary() failed, confd_errno=%d\n",
                               hnot->data.besecondary_result);
                    }
                    break;
                }
                default:
            ...
            case CONFD_NOTIF_COMMIT_PROGRESS: ;
                ...
                char subsystem[80] = "";
                char annotation[80] = "";
                time_t time = (time_t) (n.n.progress.timestamp / 1000000);
                int ms = n.n.progress.timestamp % 1000000;
                struct tm ts;
                ts = *localtime(&time);
                strftime(timestamp, sizeof(timestamp),
                         "%Y-%m-%dT%H:%M:%S", &ts);
                if (n.n.progress.type == CONFD_PROGRESS_STOP) {
                    snprintf(duration, sizeof(duration),
                             " (%lld ms)", (n.n.progress.duration / 1000));
                }
                switch (n.n.progress.type) {
                case CONFD_PROGRESS_START:
                    type = "start";
                    break;
                case CONFD_PROGRESS_STOP:
                    type = "stop";
                    break;
                case CONFD_PROGRESS_INFO:
                    type = "info";
                    break;
                }
                switch (n.n.progress.datastore) {
                case CONFD_CANDIDATE:
                    datastore = "candidate";
                    break;
                case CONFD_RUNNING:
                    datastore = "running";
                ...
                case CONFD_STARTUP:
                    datastore = "startup";
                    break;
                case CONFD_OPERATIONAL:
                    datastore = "operational";
                    break;
                case CONFD_TRANSACTION:
                    datastore = "transaction";
                    break;
                case CONFD_PRE_COMMIT_RUNNING:
                    datastore = "pre-commit-running";
                    break;
                case CONFD_INTENDED:
                    datastore = "intended";
                    break;
                ...
                if (n.n.progress.trace_id[0] != '\0') {
                    snprintf(trace_id, sizeof(trace_id),
                             " trace_id=%s", n.n.progress.trace_id);
                }
                if (n.n.progress.subsystem != NULL) {
                    snprintf(subsystem, sizeof(subsystem),
                             "%s: ", n.n.progress.subsystem);
                }
                if (n.n.progress.annotation != NULL) {
                    snprintf(annotation, sizeof(annotation),
                             ": %s", n.n.progress.annotation);
                }
                printf("progress: type=%s usid=%d tid=%d datastore=%s "
                       "context=%s%s\n %s.%d %s%s%s%s\n",
                       type,
                       n.n.progress.usid,
                       n.n.progress.tid,
                       datastore,
                       n.n.progress.context,
                       trace_id,
                       timestamp,
                       ms,
                       subsystem,
