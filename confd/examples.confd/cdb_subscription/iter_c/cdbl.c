...
static void read_head(int cdbsock, confd_value_t *headkey)
{
    ...
    if (cdb_cd(cdbsock, "/root/node-b/rf-head{%x}", headkey) != CONFD_OK)
        confd_fatal("Failed to cd");
    hp->dn = CONFD_GET_INT64(headkey);
    hp->inuse = 1;
    if (cdb_get_str(cdbsock, hp->sector_id, 244, "sector-id") !=
        CONFD_OK)
...
static void read_db(int cdbsock)
{
    ...
    int n = cdb_num_instances(cdbsock, "/root/node-b/rf-head");
    for (i=0; i<MAXH; i++) rfheads[i].inuse = 0;
    for(i=0; i<n; i++) {
        if (cdb_get(cdbsock, &key, "/root/node-b/rf-head[%d]/dn", i) !=
    ...
    switch (op) {
        case MOP_CREATED: {
            ...
            switch (CONFD_GET_XMLTAG(ctag)) {
                case root_rf_head:  { /* an rfhead was created */
        ...
        case MOP_DELETED:
            ...
            switch (CONFD_GET_XMLTAG(dtag)) {
                case root_rf_head:  { /* an rfhead was deleted */
                    /* keypath is /root/node-b/rf-head{$key}  */
                    /*              3     2      1    0     */
        ...
        if (set[0].revents & POLLIN) {
            ...
            if (reslen > 0) {
                ...
                struct xml_tag tags[] = {{root_root, root__ns},
                                         {root_node_b, root__ns},
                                         {root_rf_head, root__ns},
                                         {root_child, root__ns}};
                int tagslen = sizeof(tags)/sizeof(tags[0]);
                /* /root/node-b/rf-head/Child */
