...
static void install_keys(struct sockaddr_in *addr)
{
    ...
    if (confd_crypto_keys.aes_keys_initialized != 1)
        confd_fatal("No AES128 keys found in the confd.conf file \n");
    if (confd_crypto_keys.aes256_keys_initialized != 1)
        confd_fatal("No AES256 keys found in the confd.conf file \n");
    if (confd_crypto_keys.des3_keys_initialized != 1)
        confd_fatal("No DES3 keys found in the confd.conf file \n");
...
static int get_clear_text(struct sockaddr_in *addr, FILE *f)
{
    ...
    for(i=0; i<n; i++) {
        ...
        cdb_get_str(rsock, cstr, sizeof(cstr), "/strs/str[%d]/secret2", i);
        memset(dstr, 0, sizeof(dstr));
        confd_decrypt(cstr, strlen(cstr), dstr);
        fprintf(f, "/strs/str{%d}/secret2=$0$%s\n", nr, dstr);
        cdb_get_str(rsock, cstr, sizeof(cstr), "/strs/str[%d]/secret3", i);
        memset(dstr, 0, sizeof(dstr));
        confd_decrypt(cstr, strlen(cstr), dstr);
        fprintf(f, "/strs/str{%d}/secret3=$0$%s\n", nr, dstr);
    }
