/*********************************************************************
 * A generic maapi_get_object/objects() implementation for reading
 * data in ConfD tag+value format that can be used to for example
 * transform the data into a format not supported by
 * maapi_save_config().
 *
...
/* Begin demo code */
static int traverse_cs_nodes(int ms, int th, int j, char *path,
                             struct confd_cs_node *curr_cs_node,
                             confd_value_t *values, int keyleafs)
{
  int nkeys = 0, tot_nobjs, nobjs, curr_nobjs, i, nsubvals, ret, len, sublen;
  int tmpj;
  u_int32_t *keyptr;
  ...
  if (j > (tv_alloc_len / 2)) {
    tv_alloc_len *= 2;
    tvs = (confd_tag_value_t *) realloc(tvs,
                                     sizeof(confd_tag_value_t) * tv_alloc_len);
  }

  len = strlen(path);
  int nval = 0;
  for (cs_node = curr_cs_node; cs_node != NULL;
       cs_node = cs_node->next, nval++) {
    snprintf(&path[len], BUFSIZ - len, "/%s:%s", confd_ns2prefix(cs_node->ns),
             confd_hash2str(cs_node->tag));
    if (cs_node->info.flags & CS_NODE_IS_LIST) {
      ...
      do {
        ret = maapi_get_objects(&mc, &subvals[curr_nobjs * nsubvals], nsubvals,
                                &nobjs);
        curr_nobjs += nobjs;
      } while (ret >= 0 && mc.n != 0);
      maapi_destroy_cursor(&mc);
      nkeys = 0;
      for (keyptr = cs_node->info.keys; keyptr != NULL && *keyptr != 0;
           keyptr++) {
        nkeys++;
      }
      sublen = strlen(path);
      for (i = 0; i < tot_nobjs; i++) {
        confd_format_keypath(&path[sublen], BUFSIZ - sublen, "{%*x}", nkeys,
                             &subvals[i * nsubvals]);
        CONFD_SET_TAG_XMLBEGIN(&tvs[j], cs_node->tag, cs_node->ns); j++;
        tmpj = j;
        j = traverse_cs_nodes(ms, th, j, path, cs_node->children,
                              &subvals[i * nsubvals], nkeys);
        if (j == tmpj) {
          j--;
        } else {
      ...
      if (values != NULL) {
        ...
        confd_fatal("A root node must be a container or list node\n");
      }
    }
    path[len] = 0;
  }
