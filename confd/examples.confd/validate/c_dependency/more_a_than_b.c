...
/* Called when a-number or b-number is modified.  Always called with
   a-number's value */
                      ...
                      confd_value_t *newval)
{
    ...
    /* we validate that a-number > b-number  */
    newval_a = CONFD_GET_INT64(newval);

    /* this switch is not necessary in this case; we know that we're
       called for a-number only.  the switch is useful when the same
    ...
    case mtest_a_number:
        ...
        /* just an assertion to show that newval == /mtest/a-number */
        /* in this transaction */
        assert(CONFD_GET_INT64(newval) == a_val);

        if (newval_a <= b_val) {
            confd_trans_seterr(tctx, "a-number is <= b-number ");
            return CONFD_ERR;
        }
        if (newval_a == 88) {
    ...
    default:
        confd_trans_seterr(tctx, "Unknown tag %d",
